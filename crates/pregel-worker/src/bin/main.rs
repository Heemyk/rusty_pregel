...
use pregel_observability::{init_verbose_observer, observe, verbose_level, ObservableEvent};
...
fn parse_args(args: &[String]) -> Result<(WorkerId, String, PathBuf, usize, u16, Algorithm, TransportKind, Option<PathBuf>, Option<String>, u8), String> {
    if args.len() < 6 {
        return Err(format!(
            "Usage: {} <worker_id> <coordinator_addr> <graph_path> <worker_count> <listen_port> [--algo pagerank|cc] [--program <wasm-path>] [--transport tcp|quic] [--checkpoint-dir <path>] [--verbose[=2]|-v|-vv]",
    ...
    Ok((worker_id, coordinator_addr, graph_path, worker_count, listen_port, algo, transport, program, checkpoint_dir, verbose))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let (worker_id, coordinator_addr, graph_path, worker_count, listen_port, algo, transport, program, checkpoint_dir, verbose) =
    ...
    let partitions = load_and_partition(&graph_path, worker_count, partition_impl.as_ref(), algo)?;
