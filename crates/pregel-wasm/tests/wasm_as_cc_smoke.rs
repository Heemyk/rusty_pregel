//! Smoke test: run AssemblyScript CC WASM, verify output format.
//! Requires: cd sdk/assemblyscript && npm run asbuild:release
...
#[test]
fn wasm_as_cc_produces_outgoing_in_superstep0() {
    let wasm_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../sdk/assemblyscript/build/algo.release.wasm");
    if !wasm_path.exists() {
        eprintln!("Skipping: build AS WASM first: cd sdk/assemblyscript && npm run asbuild:release");
        return;
    }
